...
{ // Model : Head
    ...
    { // BLOCK:auth_check:20131123:권한체크
        ...
        $result = module_user_auth_check($_SESSION['mapc_user_uid'], 'edit');
        ...
        // 파일 업로드일 경우
            ...
            foreach($_FILES['post_file']['name'] as $key => $var) {
                ...
                if (move_uploaded_file($_FILES['post_file']['tmp_name'][$key], $save_dir . $file_name)) {
                    ...
                    $publish_data['message'] = "file_uploaded";

                } else {

                    $publish_data['message'] = "file_upload_error";
...
{ // View : Head
    ...
    $message  = htmlspecialchars($contents);
    $message .= $publish_data['message'];
    $message .= '<br />';
    $message .= '<a href="' . $URL['mapc']['list'] . '">success</a>';
